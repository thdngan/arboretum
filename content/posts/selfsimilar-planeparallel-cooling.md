...
One of our main assumptions is that radiative losses balance out compressional [[heat|heating]], keeping the contracting gas relatively cold. As the gas is compressed, the extra internal energy escapes as radiation, so it heats up less than gas under adiabatic compression would.
...
Here, the symbols with hats represent the scaled physical quantities in relation to the unscaled parent system by the scaling factor $\lambda$. For the transformed system to have the same form as the original one (so that self-similar solutions can exist), the constants a, b, c, d and e must satisfy:
...
g'(0) = 0, \nu(0) = 0, \tau'(0) = 0
$$
These mean there is no singularity at the origin, and no density or temperature gradient there. Substituting them into the equations gives $g_1 = \nu_0 = \nu_2 = \tau_1 = 0$. By symmetry, all odd-order terms of g and τ, and all even-order terms of ν, vanish. This simplifies the polynomial series to:
...
So $K_1$ sets both the magnitude and the sign of $g_2$. Setting $\tau_0 = 1$ in the series is equivalent to fixing $K_2$: changing $\tau_0$ and $K_2$ only rescales the system, and gives exactly the same solution in physical units.

...
const ensureFontLoaded = (font: BodyFont) => {
  ...
  // A stylesheet added by script does not hold up the first paint the way the
  // one in <head> does, so the page would draw in the fallback and then reflow
  // into the chosen face on every load. While the document is still being
  // parsed - which is when the saved choice is applied - ask for the same
  // treatment the <head> link gets. Browsers without `blocking` ignore it.
...
document.addEventListener("nav", () => {
  ...
  // them. Fetched only when the menu was opened, the panel drew in the fallback
  // and then reflowed as each face landed, so they are fetched ahead of time
  // instead: subset to the menu's own characters (see previewHref) they are
  // small enough to take in idle time. A stylesheet alone downloads nothing
  // while the panel is hidden - a browser only fetches a face once some visible
  // text uses it - so each face is asked for by name once the sheet is in.

...
function createFolderNode(
  ...
  // Upstream also opens every folder on the way to the current page. Left out on
  // purpose: the tree starts folded on every page, the current one included,
  // and only a folder the reader opens is open.
  if (!isCollapsed) {
...
async function setupExplorer(currentSlug: FullSlug) {
  ...
  for (const explorer of allExplorers) {
    ...
    if (scrollTop) {
      ...
      // skipped when the entry sits in a folded folder: scrollIntoView also
      // scrolls that folder's own overflow-hidden list, which would leave its
      // contents shifted when the reader opens it
      const activeElement = explorerUl.querySelector(".active")
      if (activeElement && !activeElement.closest(".folder-outer:not(.open)")) {
